<template>
  <div class="space-y-6">
    ...
    <div class="flex justify-between items-center">
      <h1 class="text-3xl font-bold text-gray-800">{{ t('nav.movies') }}</h1>
    ...
    <div class="bg-white p-4 rounded-lg shadow space-y-4">
      <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
        ...
        <!-- Subtitle Filter -->
        <div>
          <label class="block text-sm font-medium text-gray-700 mb-2">{{ t('media.status') }}</label>
          <select
            v-model="filters.has_subtitle"
            class="w-full px-3 py-2 border border-gray-300 rounded focus:ring-2 focus:ring-blue-500 focus:border-transparent"
            @change="loadMovies"
          >
            <option :value="undefined">All</option>
            <option :value="true">{{ t('media.hasSubtitle') }}</option>
            <option :value="false">{{ t('media.noSubtitle') }}</option>
          </select>
        </div>
      </div>
    </div>
    ...
    <div v-else-if="viewMode === 'grid'" class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-6">
      <div
        ...
        @click="goToDetail(movie.media_id)"
      >
        <!-- Poster -->
        <div class="aspect-[2/3] bg-gray-200 relative">
          <img
            v-if="movie.has_poster"
            :src="getPosterUrl(movie.media_id)"
            :alt="movie.title"
          ...
          <div
            :class="[
              ...
              getStatusColor(movie.processing_status)
            ]"
          >
            {{ t(`status.${movie.processing_status}`) }}
          </div>
        </div>

        <!-- Info -->
        <div class="p-4">
          <h3 class="font-semibold text-gray-900 truncate" :title="movie.title">
            {{ movie.title }}
          </h3>
          ...
          >
            ...
            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
              <button
                ...
                @click.stop="goToDetail(movie.media_id)"
              >
                {{ t('common.view') }}
              </button>
            </td>
    ...
    <!-- Pagination -->
    <div v-if="!loading && movies.length > 0" class="flex justify-between items-center">
...
import type { MediaItem, PaginationInfo } from '@/types/api'
...
const filters = ref({
  ...
  has_subtitle: undefined as boolean | undefined
})
...
async function loadMovies() {
  ...
  try {
    const response = await mediaApi.getMovies({
      page: pagination.value.page,
      page_size: pagination.value.page_size,
      search: filters.value.search || undefined,
      year: filters.value.year,
      has_subtitle: filters.value.has_subtitle
    })
    ...
    if (response.success && response.data) {
      ...
      error.value = response.error || 'Failed to load movies'
    }
  } catch (err: any) {
...
function goToDetail(mediaId: number) {
  router.push({ name: 'movie-detail', params: { mediaId } })
}
